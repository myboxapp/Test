...
public class ReservationUpgradeService {
     ...
     * <p>
     * Justification: case #2.2 bulk update
     */
    ...
    public void convertRecurringRule() {
        final DataSource reservationDataSource =
                DataSourceFactory.createDataSourceForFields(RESERVE_TABLE, new String[] {
                        RES_PARENT, RECURRING_RULE, RES_TYPE });
        reservationDataSource.addRestriction(Restrictions.isNotNull(RESERVE_TABLE, RES_PARENT));
        reservationDataSource.addRestriction(Restrictions.eq(RESERVE_TABLE, RES_TYPE,
            Constants.TYPE_RECURRING));
        reservationDataSource.addRestriction(Restrictions.like(RESERVE_TABLE, RECURRING_RULE,
            "<options%"));
        ...
        for (final DataRecord record : records) {
            ...
            try {
                ...
                logger.warn("Syntax Error in recurring_rule for res_parent = " + parentId,
                    exception);
            } catch (ReservationException exception) {
                logger.warn("Error converting recurring_rule for res_parent = " + parentId,
                    exception);
            }
        }
    }
    ...
    private void convertRecurringRule(final int parentId, final String recurringRule)
        ...
        if ("day".equals(recurrenceType)) {
            ...
            newRecurringRule =
                    RecurringScheduleService.getRecurrenceXMLPattern(
                        RecurringSchedulePattern.TYPE_WEEK, 1, -1, getDaysOfTheWeek(weeklyElement),
                        -1, -1, -1);
        } else if ("month".equals(recurrenceType)) {
            final Element monthlyElement = rootElement.element("monthly");
            newRecurringRule =
                    RecurringScheduleService.getRecurrenceXMLPattern(
                        RecurringSchedulePattern.TYPE_MONTH, 1, -1,
                        getDaysOfTheWeek(monthlyElement), -1, getWeekOfMonth(monthlyElement), -1);
        } else {
    ...
    private int parseInterval(final String value, final int parentId) {
        ...
        try {
            ...
            logger.info("Invalid interval in old recurrence format: '" + value
                    + "' for res_parent=" + parentId
                    + ". Default to interval=1 for conversion.");
        }
        return result;
    }
    ...
    private int getWeekOfMonth(final Element element) {
        ...
        for (int index = 0; index < weekAttributeNames.size(); ++index) {
            if (Boolean.parseBoolean(element.attributeValue(UNDERSCORE
                    + weekAttributeNames.get(index)))) {
