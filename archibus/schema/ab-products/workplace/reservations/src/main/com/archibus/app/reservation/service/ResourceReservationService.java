...
public class ResourceReservationService {       
    ...
    public DataRecord saveResourceReservation(final DataRecord reservation,  
        ...
        final ResourceReservation resourceReservation =
                (ResourceReservation) this.resourceReservationDataSource.convertRecordToObject(reservation);   

        // make sure the date of time values are set to 1899 
        resourceReservation.setStartTime(new Time(TimePeriod.clearDate(
                resourceReservation.getStartTime()).getTime()));
        resourceReservation.setEndTime(new Time(TimePeriod.clearDate(
                resourceReservation.getEndTime()).getTime()));       
        ...
        ReservationServiceHelper.validateEmails(resourceReservation);
 
        // check the start and end time window
        ReservationServiceHelper.checkResourceAllocations(resourceReservation);

        List<ResourceReservation> createdReservations = null;

        if (ReservationServiceHelper.isNewRecurrence(resourceReservation)) {
            // prepare for new recurring reservation and correct the end date
            final Recurrence recurrence = ReservationServiceHelper.prepareNewRecurrence(resourceReservation);

            // save and return the generated reservation instances
            createdReservations = resourceReservationServiceHelper.saveRecurringResourceReservation(
                            resourceReservation, recurrence); 
             
            // create or update the work request 
            this.workRequestService.createWorkRequest(resourceReservation, true);     
        } else {
            resourceReservationServiceHelper.saveReservation(resourceReservation); 
            // create or update the work request
        ...
        if (reservationId == null || parentId == null || parentId.equals(reservationId)) {
            ...
            for (ResourceReservation createdReservation : createdReservations) {
                EmailNotificationHelper.sendNotifications(createdReservation.getReserveId(), null, null);
            }
        }
        ...
        reservation.setValue(this.resourceReservationDataSource.getMainTableName() + Constants.DOT
                + Constants.RES_ID, resourceReservation.getReserveId());
    ...
    public void cancelResourceReservation(final Integer reservationId, final String comments) {
        ...
        if (reservationId != null && reservationId > 0) {
            ...
            resourceReservation = this.resourceReservationDataSource.getActiveReservation(reservationId, null);
        } else {
        ...
        if (resourceReservation != null) {
            ...
            // send notification
            EmailNotificationHelper.sendNotifications(resourceReservation.getReserveId(), null, comments);
        }
        ReservationsContextHelper.ensureResultMessageIsSet();
    }
     ...
     */
    public List<Integer> cancelMultipleResourceReservations(final DataSetList reservations, final String comments) {
        ...
        for (DataRecord record : reservations.getRecords()) {
            ...
            final ResourceReservation resourceReservation = this.resourceReservationDataSource.get(reservationId);
     ...
     */
    public List<Integer> cancelRecurringResourceReservation(final Integer reservationId, final String comments) {
        final List<Integer> failures = new ArrayList<Integer>();
        if (reservationId != null && reservationId > 0) {
            ...
            if (reservationId.equals(reservation.getParentId()) && failedReservations.isEmpty()) {
                ...
                for (IReservation cancelledReservation : cancelledReservations) {
                    EmailNotificationHelper.sendNotifications(cancelledReservation.getReserveId(), null, comments);
                }
            }
    ...
    public Double calculateTotalCost(final DataRecord reservation,   final DataSetList resources,
        ...
        final ResourceReservation resourceReservation =
                (ResourceReservation) this.resourceReservationDataSource.convertRecordToObject(reservation);
        // make sure the date of time values are set to 1899 
        resourceReservation.setStartTime(new Time(TimePeriod.clearDate(resourceReservation.getStartTime()).getTime()));
        resourceReservation.setEndTime(new Time(TimePeriod.clearDate(resourceReservation.getEndTime()).getTime()));
        ...
        return this.resourceReservationDataSource.calculateCosts(resourceReservation) * numberOfOccurrences;
    }
    ...
    public void copyResourceReservation(final int reservationId, final String reservationName,
            final Date startDate) {
        final ResourceReservation sourceReservation =
                this.resourceReservationDataSource.getActiveReservation(reservationId, null);
