...
public class ResourceReservationServiceHelper {     
     ...
     */
    public void markRecurring(final ResourceReservation reservation,
        ...
        for (AbstractReservation existingReservation : existingReservations) {
        ...
        resourceReservationDataSource.markRecurring(reservation, parentId, reservation
        ...
        for (ResourceReservation existingReservation : existingReservations) {
            resourceReservationDataSource.markRecurring(existingReservation, parentId,
    ...
    public List<ResourceReservation> saveRecurringResourceReservation(
        ...
        final List<ResourceReservation> existingReservations =
                this.resourceReservationDataSource.getByParentId(resourceReservation.getParentId(),
                    null, resourceReservation.getStartDate());

        if (existingReservations == null) { 
            insertRecurringReservations(recurrence,
                    savedReservations, resourceReservation); 
        } else {
            ReservationServiceHelper.updateExistingReservations(resourceReservationDataSource,
                    savedReservations, resourceReservation, existingReservations);
        }
     ...
     * @param resourceReservation the resource reservation
     */
    public void insertRecurringReservations(
        ...
        if (recurrence instanceof AbstractIntervalPattern) {
            ...
            final String requestorTimeZone = resourceReservation.getTimeZone();
            // save the first base reservation
            saveReservation(resourceReservation);
            // Set its parent reservation ID.                
            final int parentId = resourceReservation.getParentId() == null 
                    ? resourceReservation.getReserveId() : resourceReservation.getParentId();  
            this.resourceReservationDataSource.markRecurring(resourceReservation,
                    parentId, pattern.toString());

            // get the saved copy with the proper time zone
            final ResourceReservation activeReservation =
                    this.resourceReservationDataSource.getActiveReservation(
                            resourceReservation.getReserveId(), requestorTimeZone);
